...
page('regulations/kaiko/index.html','懐古レギュ｜使用可能パーツ一覧',crumb('<a href="../index.html">レギュレーション</a><span>/</span>懐古レギュ') + f'''<section class="page-heading"><div><p class="eyebrow">NOSTALGIC CLASS</p><h1>懐古レギュ</h1><p class="intro">あの頃のパーツで、マシンをつくろう。</p></div><span class="roundel">29<small>PARTS</small></span></section><section aria-labelledby="parts-heading"><div class="section-heading"><h2 id="parts-heading">使用可能パーツ一覧</h2><span>全29点</span></div><p class="note">指定カタログで印の付いたパーツを掲載しています。画像をタップすると、商品名・仕様を大きく確認できます。<br>画像内の価格・商品情報は、掲載元カタログ当時のものです。</p><div class="parts-grid">{cards}</div></section>''','regulations',2)
